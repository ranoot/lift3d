...
// clustering HDBSCAN* is known for, using the standard CONDENSED TREE of
// Campello/McInnes with either EXCESS OF MASS (EOM) or LEAF cluster selection.
// Kept deliberately parlay-free (plain std types only) so it is unit-testable on
// synthetic linkages and does not leak the heavy parallel runtime into its includers.
//
...
// `leaf_selection` false => EOM (default: fewer, larger, more stable clusters); true
// => select the condensed tree's LEAF clusters (more, smaller, homogeneous ones).
